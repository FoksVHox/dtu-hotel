...
{
    /**
     * Booking ID to exclude from overlap checks (null for new bookings).
     */
    ...
    protected function validateGuestsPresent(Validator $validator): void
    {
    ...
    protected function validateRoomAvailability(Validator $validator): void
    {
        ...
        $excludeId = $this->excludedBookingId();

        $maintenanceConflict = Booking::when($excludeId, fn (Builder $q) => $q->where('id', '!=', $excludeId))
        ...
        $overlapConflict = Booking::when($excludeId, fn (Builder $q) => $q->where('id', '!=', $excludeId))
