...
export default function BuildingsStep({ buildings, onAdvance, onBack }: BuildingsStepProps) {
    ...
    const form = useForm<{ buildings: Array<{ name: string; floors_count: number }> }>({ buildings: initial });
    ...
    return (
        <form onSubmit={submit} className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-3">
                {form.data.buildings.map((b, idx) => (
                    <div key={idx} className="flex items-end gap-3 rounded border border-neutral-200 p-4">
                        <label className="flex flex-1 flex-col gap-1.5">
                            ...
                            <input type="text" value={b.name} onChange={(e) => updateBuilding(idx, 'name', e.target.value)} className="h-11 rounded border border-neutral-300 px-3 focus:border-black focus:outline-none focus:ring-1 focus:ring-black" />
                        </label>
                        <label className="flex w-32 flex-col gap-1.5">
                            <span className="text-sm font-medium">Floors</span>
                            <input type="number" min={1} max={20} value={b.floors_count} onChange={(e) => updateBuilding(idx, 'floors_count', Number(e.target.value))} className="h-11 rounded border border-neutral-300 px-3 focus:border-black focus:outline-none focus:ring-1 focus:ring-black" />
                        </label>
                        {form.data.buildings.length > 1 && (
                            <button type="button" onClick={() => removeBuilding(idx)} className="flex size-11 items-center justify-center rounded border border-neutral-300 hover:bg-neutral-100" aria-label="Remove building">
                                <X className="size-4" />
                            </button>
                        )}
                    </div>
                ...
                <button type="button" onClick={addBuilding} disabled={form.data.buildings.length >= 10} className="flex items-center justify-center gap-2 self-start rounded border border-dashed border-neutral-400 px-4 py-2 text-sm hover:border-black disabled:opacity-50">
